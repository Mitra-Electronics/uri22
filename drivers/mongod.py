...
def raise_not_found(request):
    raise HTTPException(
        status_code=404, detail=f"URL '{request}' doesn't exist")
...
def create_acc(data: dict):
    ...
    if acc.find_one(data["email"]) != None:
        raise HTTPException(
            status_code=266, detail=data["email"]+" already used")
    data.pop("password")
    acc.insert_one(data)
...
def fetch_url(url: str):
    ...
    if dat["payment_required"]:
        raise HTTPException(
            status_code=402, detail="Payment required to access URL")
    return dat["url"]


def edit_url(data: dict, token: str):
    account = get_acc(token)
    c = urls.update_one({"referral_id": data["referral_id"], "account_id": account["_id"]}, {
                        "$set": {"url": data["url"]}})
...
def del_url(data: dict, token: str):
    account = get_acc(token)
    acc.update_one({"_id": account["_id"]}, {
                   "$set": {"referrals_created": account["referrals_created"]-1}})
    c = urls.delete_one(
        {"referral_id": data["referral_id"], "account_id": account["_id"]}).deleted_count
    if c == 0:
        raise HTTPException(status_code=404, detail="URL does not exist")
    return c
